...
test('sharing AB Club emits an explicit, instance-scoped brand entry path', async () => {
  ...
  try {
    ...
    assert.equal(share.title, 'AB Club · 你的名片，连接世界');
    const entry = new URL(share.path, 'https://example.invalid');
    assert.equal(entry.pathname, '/pages/discover/index');
    assert.equal(entry.searchParams.get('entry'), 'brand');
    assert.ok(entry.searchParams.get('entry_id').startsWith(`${Date.now().toString(36)}-`));
    assert.equal(share.imageUrl, '/assets/brand/ab-club-brand-share.jpg');
  } finally {
    Date.now = realNow;
    delete globalThis.Page;
    delete globalThis.getApp;
