...
test('successful response keeps the bearer only in memory and emits a token-only share query', async () => {
  ...
  try {
    ...
    assert.equal(page.data.shareHint, '', 'normal native sharing should not add developer-facing status copy');
    assert.doesNotMatch(page.data.shareHint, /分享成功|发送成功/, 'the native callback cannot claim delivery');
...
test('owner card instances keep independent secrets and stop a token revoked in the manager', async () => {
  ...
  try {
    ...
    assert.match(pageA.data.shareHint, /已停止分享/);
