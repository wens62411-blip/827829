...
test('two card-share page instances resolve and forward only their own bearer', async () => {
  ...
  try {
    ...
    assert.equal(pageA.onShareAppMessage.call(pageA).imageUrl, '/assets/brand/ab-club-share-safe-cover.jpg');
    assert.equal(pageB.onShareAppMessage.call(pageB).imageUrl, '/assets/brand/ab-club-share-safe-cover.jpg');
  } finally {
...
test('explicit demo share opens only in offline demo, keeps theme, and never resolves a cloud bearer', async () => {
  ...
  try {
    ...
    assert.equal(forward.imageUrl, '/assets/brand/ab-club-share-safe-cover.jpg');
...
test('a demo snapshot cold start restores only its explicit public labels and fails closed after tampering', async () => {
  ...
  try {
    ...
    assert.equal(received.data.demoFields.some((field) => field.key === 'email'), false);
...
test('a local identity cold start restores only public user input and never carries private contacts or demo fixtures', async () => {
  ...
  try {
    ...
    assert.equal(received.onShareAppMessage.call(received).imageUrl, '/assets/brand/ab-club-share-safe-cover.jpg');
