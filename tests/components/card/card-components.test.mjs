...
test('profile custom theme, labels and gallery stay owner-only and never imitate verification', () => {
  ...
  assert.match(template, /wx:if="\{\{isSelf && safeSelectedLabels\.length\}\}" class="profile-card__section profile-card__selected-labels"/);
  assert.match(template, /wx:if="\{\{isSelf && safeGallery\.length\}\}" class="profile-card__section profile-card__gallery-section"/);
  assert.match(template, /aria-label="本人自选标签，未经人工认证"/);
